...
public interface SistemaDesarrolloRepository extends JpaRepository<SistemaEntity, Long>, JpaSpecificationExecutor<SistemaEntity> {
    ...
    @Query("""
            SELECT s FROM DesarrolladorSistemaEntity s
            WHERE s.eliminado = false
              AND (s.usuarioCreador = :usuario
                   OR (s.usuarioCreador IS NULL AND s.responsableTecnico = :usuario))
            """)
    List<SistemaEntity> findByPropietarioAndEliminadoFalse(@Param("usuario") String usuario);
    ...
    @Query("""
            ...
            WHERE s.eliminado = false
              ...
              AND (s.usuarioCreador = :usuario
                   OR (s.usuarioCreador IS NULL AND s.responsableTecnico = :usuario))
            """)
    List<SistemaEntity> findByPropietarioAndEstadoAndEliminadoFalse(
