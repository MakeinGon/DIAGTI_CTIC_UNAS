...
function cargarRolesParaSelects() {
    return fetch(`${API_BASE}/roles`, { cache: 'no-store' })
...
function llenarSelectRoles(select, esFiltro) {
    ...
    const roles = esFiltro ? rolesDisponibles : rolesCompatiblesConArea(
        document.getElementById('select-usuario-area')?.value
    );
    roles.forEach(rol => {
...
function rolesCompatiblesConArea(area) {
    const a = normalizarRol(area);
    let permitidos = [];
    if (a.includes('desarrollo')) permitidos = ['desarrollo', 'desarrollador', 'area de desarrollo'];
    else if (a.includes('infraestructura')) permitidos = ['infraestructura', 'area de infraestructura'];
    else if (a.includes('direccion academica')) permitidos = ['funcional', 'responsable funcional'];
    else if (a.includes('control interno')) permitidos = ['validacion', 'validador', 'validador tecnico', 'validador ctic', 'auditor'];
    else if (a.includes('rectorado')) permitidos = ['directivo'];
    else if (a.includes('administracion')) permitidos = ['admin', 'administrador', 'administrador ctic'];
    return rolesDisponibles.filter(rol => permitidos.includes(normalizarRol(rol.nombre)));
}
...
function cargarUsuarios() {
    return fetch(`${API_BASE}/usuarios`, { cache: 'no-store' })
        .then(res => manejarError(res))
...
function actualizarUsuariosDesdeBase() {
    const boton = document.getElementById('btn-actualizar-usuarios');
    if (boton) {
        boton.disabled = true;
        boton.textContent = 'Actualizando...';
    }
    return Promise.all([cargarRolesParaSelects(), cargarUsuarios()])
        .finally(() => {
            if (boton) {
...
function limpiarFormularioUsuario() {
    ...
    document.getElementById('input-usuario-password-confirm').value = '';
    document.getElementById('usuario-password-hint').textContent =
        'La contraseña es obligatoria para crear la cuenta y se guarda cifrada.';
...
function editarUsuario(fila) {
    ...
    document.getElementById('input-usuario-password-confirm').value = '';
    document.getElementById('usuario-password-hint').textContent =
        'Déjela en blanco para conservar la contraseña actual.';
...
function guardarUsuario() {
    ...
    const passwordConfirm = document.getElementById('input-usuario-password-confirm').value;
    ...
    if (!esEdicion && !password) {
        mostrarErrorFormulario('usuario-form-error', 'Crea una contraseña para que el nuevo usuario pueda iniciar sesión.');
        return;
    }
    if (password && password.length < 6) {
        mostrarErrorFormulario('usuario-form-error', 'La contraseña debe tener al menos 6 caracteres.');
        return;
    }
    if (password !== passwordConfirm) {
        mostrarErrorFormulario('usuario-form-error', 'Las contraseñas no coinciden.');
        return;
    }
...
document.getElementById('select-usuario-area')?.addEventListener('change', function() {
    llenarSelectRoles(document.getElementById('select-usuario-rol'), false);
});
