/* ============================================================
   dashboard-riesgos.component.js
   Fusión de: js/sidebar.js + js/dashboard-riesgos.js
   ============================================================ */
...
const state = {
    data: [], filters: { dateFrom:"2025-01-01", dateTo:"2026-12-31", area:"all", category:"all", level:"all", status:"all", search:"" },
    flag: null, sort: { key:"level", direction:"asc" }, page:1, pageSize:10, loading:false
};
const el = {};
let searchTimer;
...
async function loadData() {
    ...
    } catch(error) { console.error("[DIAGTI] Riesgos:",error); showState("error",error.name==="AbortError"?"La consulta excedió el tiempo máximo.":error.message); }
...
function filteredRisks(ignore={}) {
    ...
    return state.data.filter(r=>{
        if (!ignore.date && r.detectedAt && (r.detectedAt < state.filters.dateFrom || r.detectedAt > state.filters.dateTo)) return false;
        ...
        if (q && !normalize(`${r.code} ${r.title} ${r.area} ${r.category}`).includes(q)) return false;
        return true;
    });
}
...
function renderActiveFilters() {
    ...
    if(state.filters.level!=="all") chips.push(["level",`Criticidad: ${RISK_LABELS.level[state.filters.level]}`]);
...
function renderTable(risks) {
    ...
    el.tableBody.innerHTML=pageRows.map(r=>`<tr><td><strong>${escapeHTML(r.code)}</strong></td><td>${escapeHTML(r.title)}</td><td>${escapeHTML(r.area)}</td><td>${escapeHTML(r.category)}</td><td>${badge(RISK_LABELS.level[r.level],r.level==="critico"?"danger":r.level==="advertencia"?"warning":"success")}</td><td>${badge(RISK_LABELS.status[r.status],r.status==="controlado"?"success":r.status==="mitigacion"?"info":"warning")}</td><td><button class="analytics-detail-button" data-risk-detail="${escapeHTML(r.id)}">Ver detalle</button></td></tr>`).join("");
...
function openDetail(id) {
    ...
    el.dialogContent.innerHTML=`<div class="analytics-dialog__grid"><div class="analytics-dialog__item"><span>Área</span><strong>${escapeHTML(r.area)}</strong></div><div class="analytics-dialog__item"><span>Categoría</span><strong>${escapeHTML(r.category)}</strong></div><div class="analytics-dialog__item"><span>Criticidad</span><strong>${RISK_LABELS.level[r.level]}</strong></div><div class="analytics-dialog__item"><span>Etapa</span><strong>${escapeHTML(r.stage)}</strong></div><div class="analytics-dialog__item"><span>Detectado</span><strong>${formatDate(r.detectedAt)}</strong></div><div class="analytics-dialog__item"><span>Estado</span><strong>${RISK_LABELS.status[r.status]}</strong></div></div><div class="analytics-dialog__recommendation"><strong>Acción preventiva:</strong> ${escapeHTML(r.recommendation)}</div>`;
    typeof el.detailDialog.showModal==="function"?el.detailDialog.showModal():el.detailDialog.setAttribute("open","");
}

function resetFilters() {
    state.filters={dateFrom:"2025-01-01",dateTo:"2026-12-31",area:"all",category:"all",level:"all",status:"all",search:""}; state.flag=null; state.page=1;
    el.filterDateFrom.value=state.filters.dateFrom; el.filterDateTo.value=state.filters.dateTo; el.filterArea.value="all"; el.filterCategory.value="all"; el.filterLevel.value="all"; el.filterStatus.value="all"; el.filterSearch.value=""; render();
}
function bind() {
    const bindings=[[el.filterArea,"area"],[el.filterCategory,"category"],[el.filterLevel,"level"],[el.filterStatus,"status"]];
    bindings.forEach(([control,key])=>control.addEventListener("change",e=>{state.filters[key]=e.target.value;state.page=1;render();}));
    [el.filterDateFrom,el.filterDateTo].forEach(control=>control.addEventListener("change",()=>{
    ...
    document.addEventListener("click",event=>{
        ...
        const remove=event.target.closest("[data-remove-risk-filter]"); if(remove){const key=remove.dataset.removeRiskFilter;if(key==="flag")state.flag=null;else{state.filters[key]=key==="search"?"":"all";const map={area:el.filterArea,category:el.filterCategory,level:el.filterLevel,status:el.filterStatus,search:el.filterSearch};if(map[key])map[key].value=state.filters[key];}state.page=1;render();}
    });
