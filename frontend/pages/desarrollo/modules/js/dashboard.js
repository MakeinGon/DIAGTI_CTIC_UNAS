...
function renderDashboard() {
    ...
    if (observados > 0 || borradores > 0) {
        ...
        greetingAction.href = observados > 0 ? 'subsanar-observaciones.html' : '#pending-section';
    } else {
    ...
    if (pendientes.length === 0) {
        ...
        pendientes.forEach(s => {
            ...
            if (s.estado === 'Borrador' && porcentaje < 100) {
                ...
                btnUrl = `subsanar-observaciones.html`;
            }
            html += `
            ...
            html += `
                <div class="obs-item">
                    <div class="obs-header">
                        ...
                        <a href="subsanar-observaciones.html" class="btn btn-warning btn-sm">🔴 Ir a Observaciones</a>
                    </div>
...
async function actualizarDashboardDesdeBackend() {
    try {
        if (window.DIAGTIFlujo) {
            await window.DIAGTIFlujo.sincronizarColeccion('diagti_sistemas', 'DESARROLLO', getSistemas());
        }
    } catch (error) {
        console.error('No se pudieron sincronizar los estados de Desarrollo:', error);
    }
    renderDashboard();
}

document.addEventListener('DOMContentLoaded', function () {
    actualizarDashboardDesdeBackend();
    document.querySelectorAll('.nav-item').forEach(item => {
        if (item.getAttribute('href') === 'dashboard.html') item.classList.add('active');
    });
    window.addEventListener('focus', actualizarDashboardDesdeBackend);
    window.setInterval(() => { if (!document.hidden) actualizarDashboardDesdeBackend(); }, 15000);
});
