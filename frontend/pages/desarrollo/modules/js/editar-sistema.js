...
const API_URL = '/api/desarrollador/editar-sistema';
...
async function enviarValidacionEdit() {
    ...
    try {
        ...
        if (result && result.exito) {
            const sistema={
                id,codigo:document.getElementById('edit-codigo')?.value||`DES-${id}`,
                nombre:document.getElementById('edit-nombre')?.value||'Sistema sin nombre',
                area:document.getElementById('edit-area')?.value||'CTIC UNAS',
                criticidad:document.getElementById('edit-criticidad')?.value||'',
                lenguaje:document.getElementById('edit-lenguaje')?.value||'',
                arquitectura:document.getElementById('edit-arquitectura')?.value||'',
                motor_bd:document.getElementById('edit-motor')?.value||''
            };
            await window.DIAGTIFlujo.enviar({codigoSistema:sistema.codigo,nombreSistema:sistema.nombre,areaOrigen:'DESARROLLO',areaUsuaria:sistema.area,responsable:window.DIAGTIFlujo.sesion().nombreCompleto||'Desarrollo CTIC',comentario:'Sistema editado y enviado por Desarrollo',datos:sistema});
            alert('✅ ' + result.mensaje);
            window.location.href = 'mis-sistemas.html';
        } else {
            alert('❌ Error: ' + (result?.mensaje || 'Error desconocido'));
