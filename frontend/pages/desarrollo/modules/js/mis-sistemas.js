...
function mensajeErrorDesarrollo(error) {
    const texto = String(error?.message || error || 'Error desconocido').trim();
    try {
...
async function persistirAvanceDesarrollo(sistema) {
    if (!window.DIAGTIFlujo || !sistema?.codigo || !sistema?.nombre) return;
    const registro = {
    ...
    try {
        if (['Subsanado', 'Corregido'].includes(sistema.estado)) await window.DIAGTIFlujo.guardarCorreccion(registro);
        else await window.DIAGTIFlujo.guardarBorrador(registro);
        return true;
    } catch (error) {
        console.error('No se pudo persistir el avance de Desarrollo:', error);
        throw new Error(mensajeErrorDesarrollo(error));
    }
}
...
let codigoNuevoReservado = null;

/**
 * Calcula el siguiente código a partir del mayor SISnnn existente. Usar la
 * cantidad de filas era incorrecto porque los códigos pueden no ser
 * consecutivos o pertenecer a distintos desarrolladores.
 */
function generarCodigoDisponible() {
    const usados = new Set();
    let mayor = 0;

    sistemas.forEach(sistema => {
        const codigo = String(sistema.codigo || sistema.id || '').trim().toUpperCase();
        if (!codigo) return;
        usados.add(codigo);
        const coincidencia = codigo.match(/^SIS(\d+)$/);
        if (coincidencia) mayor = Math.max(mayor, Number(coincidencia[1]));
    });

    let numero = mayor + 1;
    let candidato = `SIS${String(numero).padStart(3, '0')}`;
    while (usados.has(candidato)) {
        numero += 1;
        candidato = `SIS${String(numero).padStart(3, '0')}`;
    }
    return candidato;
}

function esConflictoDePropietario(error) {
    return mensajeErrorDesarrollo(error)
        .toLowerCase()
        .includes('pertenece a otro usuario');
}

/**
 * Persiste un sistema nuevo. Si otro usuario registró el mismo código unos
 * segundos antes, actualiza el inventario del área, genera el siguiente código
 * libre y reintenta una sola vez sin tocar el registro ajeno.
 */
async function persistirSistemaNuevo(nuevoSistema) {
    try {
        await persistirAvanceDesarrollo(nuevoSistema);
        return nuevoSistema;
    } catch (error) {
        if (!esConflictoDePropietario(error) || !window.DIAGTIFlujo) throw error;

        sistemas = await window.DIAGTIFlujo.sincronizarColeccion(
            SISTEMAS_KEY,
            'DESARROLLO',
            sistemas
        );
        const nuevoCodigo = generarCodigoDisponible();
        const reintentado = { ...nuevoSistema, id: nuevoCodigo, codigo: nuevoCodigo };
        codigoNuevoReservado = nuevoCodigo;
        const campoCodigo = document.querySelector('.form-group[data-campo="codigo"] input');
        if (campoCodigo) campoCodigo.value = nuevoCodigo;

        await persistirAvanceDesarrollo(reintentado);
        return reintentado;
    }
}
...
function renderizarTabla(lista) {
    ...
    data.forEach(s => {
        ...
        const usuarioActual = window.DIAGTIFlujo?.sesion?.().username || '';
        const propietario = s.usuario_origen || s.propietario_area || '';
        const puedeGestionar = !propietario || !usuarioActual ||
            String(propietario).toLowerCase() === String(usuarioActual).toLowerCase();

        // Todos los integrantes ven la información; solo el propietario
        // conserva las acciones que modifican el registro.
        let botonesAccion = `<button class="btn btn-ghost btn-sm" onclick="verSistema('${s.id}')">👁️ Ver</button>`;

        if (puedeGestionar && estado === 'Borrador') {
        ...
        } else if (puedeGestionar && estado === 'Observado') {
            botonesAccion += `<a href="subsanar-observaciones.html" class="btn btn-warning btn-sm">⚠️ Corregir</a>`;
        } else if (puedeGestionar && (estado === 'Subsanado' || estado === 'Corregido')) {
            botonesAccion += `<button class="btn btn-azul btn-sm" onclick="enviarAValidacionDirecto('${s.id}')" style="margin-left:4px;">🚀 Reenviar a Validación</button>`;
        }
        ...
        html += `
            <tr>
                ...
                <td>${fecha}</td>
                <td>${propietario || s.responsable_tecnico || 'Área de Desarrollo'}</td>
                <td>
...
function abrirModalRegistro() {
    ...
    evidenciasSubidas = [];
    urlsAgregadas = [];
    codigoNuevoReservado = generarCodigoDisponible();
...
function renderizarModalContenido(sistema, soloLectura = false, modoCorregir = false) {
    ...
    const codigo = data.codigo || codigoNuevoReservado || generarCodigoDisponible();
...
// ============================================================
async function guardarSistemaModal(esEnvioFinal = false, cerrarAlFinal = true, silencioso = false) {
    if (modoModal === 'ver') {
        if (cerrarAlFinal) cerrarModalSistema();
        return null;
    }
    ...
    let sistemaGuardado = null;
    if (modoModal === 'registrar') {
        const codigoFormulario = document
            .querySelector('.form-group[data-campo="codigo"] input')
            ?.value?.trim()?.toUpperCase();
        const nuevoId = codigoFormulario || codigoNuevoReservado || generarCodigoDisponible();
        let nuevoSistema = {
            id: nuevoId,
            codigo: nuevoId,
            nombre,
        ...
        try {
            if (!esEnvioFinal) nuevoSistema = await persistirSistemaNuevo(nuevoSistema);
        } catch (error) {
            if (silencioso) throw error;
            alert('❌ No se guardó el sistema en PostgreSQL. Puedes corregir o reintentar sin cerrar la ventana: '
                + mensajeErrorDesarrollo(error));
            return null;
        }

        sistemas.push(nuevoSistema);
        sistemaGuardado = nuevoSistema;
        guardarSistemas(sistemas);
        if (!silencioso) alert(esEnvioFinal ? '✅ El sistema ha sido enviado a revisión técnica correctamente.' : '✅ Sistema registrado correctamente como BORRADOR.');
        ...
        if (index !== -1) {
            ...
            if (esEnvioFinal) {
                sistemaActualizado.estado = 'Enviado';
            } else if (modoModal === 'corregir') {
                sistemaActualizado.estado = 'Subsanado';
                sistemaActualizado.observaciones_validador = [];
            }

            try {
                if (!esEnvioFinal) await persistirAvanceDesarrollo(sistemaActualizado);
            } catch (error) {
                if (silencioso) throw error;
                alert('❌ No se guardaron los cambios en PostgreSQL. Puedes reintentar sin cerrar la ventana: '
                    + mensajeErrorDesarrollo(error));
                return null;
            }

            sistemas[index] = sistemaActualizado;
            sistemaGuardado = sistemas[index];
            guardarSistemas(sistemas);
            if (!silencioso) alert('✅ Cambios guardados correctamente.');
        }
    }
    if (!silencioso && esEnvioFinal && modoModal !== 'registrar') {
...
// ============================================================
async function registrarSolicitudValidadorDesarrollo(sistema) {
    const session = JSON.parse(localStorage.getItem('diagti_session') || '{}');
    if (window.DIAGTIFlujo) {
        return window.DIAGTIFlujo.enviar({
            codigoSistema: sistema.codigo || `DES-${sistema.id}`,
            nombreSistema: sistema.nombre || 'Sistema sin nombre',
            areaOrigen: 'DESARROLLO',
            areaUsuaria: sistema.area_usuaria || sistema.area || 'CTIC UNAS',
            responsable: session.nombreCompleto || sistema.responsable_tecnico || 'Área de Desarrollo',
            comentario: sistema.estado === 'Observado' || sistema.estado === 'Subsanado'
                ? 'Observaciones de desarrollo subsanadas y reenviadas'
                : 'Información de desarrollo enviada para validación técnica',
            datos: sistema
        });
    }
    const response = await fetch('/api/flujo-validacion/solicitudes', {
        method: 'POST',
        ...
        body: JSON.stringify({
            ...
            areaUsuaria: sistema.area_usuaria || sistema.area || 'CTIC UNAS',
            responsable: session.nombreCompleto || sistema.responsable_tecnico || 'Área de Desarrollo',
            usuarioOrigen: session.username || session.nombreCompleto || 'Área de Desarrollo',
            comentario: 'Información de desarrollo enviada para validación técnica',
            datos: sistema
        })
    });
...
async function validateFinalSubmit() {
    ...
    // --- 6. Todas las validaciones pasaron → guardar primero en PostgreSQL ---
    const estadoAnterior = sistemaEnEdicion?.estado;
    let enviado = null;
    try {
        enviado = await guardarSistemaModal(false, false, true);
        if (!enviado) throw new Error('No se pudo preparar la información del sistema');
        await registrarSolicitudValidadorDesarrollo(enviado);

        const guardados = getSistemas();
        const confirmado = guardados.find(s => s.id === enviado.id);
        if (confirmado) {
            confirmado.estado = 'Enviado';
            guardarSistemas(guardados);
        }
    } catch (error) {
        const guardados = getSistemas();
        const fallido = guardados.find(s => s.id === enviado?.id);
        if (fallido) {
            fallido.estado = estadoAnterior || 'Borrador';
            guardarSistemas(guardados);
        }
        if (enviado) {
            // El borrador sí quedó persistido. Convertir el modal a edición
            // evita duplicarlo si el usuario pulsa "Enviar" nuevamente.
            modoModal = 'editar';
            sistemaEnEdicion = JSON.parse(JSON.stringify(enviado));
            codigoNuevoReservado = null;
        }
        alert('❌ No se envió. La ventana permanece abierta para que puedas corregir o reintentar: '
            + mensajeErrorDesarrollo(error));
        return;
    }
...
// INICIALIZACIÓN
// ============================================================
async function sincronizarEstadosDesarrollo(mostrarError = false) {
    if (!window.DIAGTIFlujo) return;
    try {
        sistemas = await window.DIAGTIFlujo.sincronizarColeccion(SISTEMAS_KEY, 'DESARROLLO', getSistemas());
        renderizarTabla();
        poblarSelectResponsables();
    } catch (error) {
        if (mostrarError) alert('No se pudieron actualizar los estados: ' + error.message);
        console.error('Error sincronizando Desarrollo:', error);
    }
}

document.addEventListener('DOMContentLoaded', async function () {
    sistemas = getSistemas();
    await sincronizarEstadosDesarrollo();
    renderizarTabla();
    ...
    poblarSelectResponsables();
    window.addEventListener('focus', () => sincronizarEstadosDesarrollo());
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) sincronizarEstadosDesarrollo();
    });
    window.setInterval(() => {
        if (!document.hidden) sincronizarEstadosDesarrollo();
    }, 15000);
    ...
    window.enviarAValidacionDirecto = async function (id) {
        if (confirm('¿Estás seguro de que deseas enviar este sistema a validación técnica? Ya no podrás editarlo.')) {
            ...
            if (index !== -1) {
                try {
                    await registrarSolicitudValidadorDesarrollo(sistemas[index]);
                    sistemas[index].estado = 'Enviado';
                    guardarSistemas(sistemas);
                    alert('✅ El sistema ha sido guardado en PostgreSQL y enviado al Validador.');
                } catch (error) {
                    alert('❌ No se envió; puede reintentar. Detalle: ' + error.message);
                }
                sistemas = getSistemas();
                renderizarTabla();
            }
